...
do
  ...
  cont_id=`sudo docker ps -q -f name=${cont_name}`
  echo "1st check: $cont_id"
  if [ -z ${cont_id} ]; then
    cont_id=`sudo docker ps -aq -f status=exited -f name=${cont_name}`
    echo 2nd check: $cont_id
    if [ -n "$cont_id" ]; then
      echo "Found existing container ${cont_name}, starting it..."
      cont_id=`sudo docker start ${cont_name}`
    else
      # run your container
