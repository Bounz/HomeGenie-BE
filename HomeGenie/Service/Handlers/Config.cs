...
namespace HomeGenie.Service.Handlers
{
    public class Config
    {
        ...
        public void ProcessRequest(MigClientRequest request)
        {
            ...
            {
                case string cmd when cmd.StartsWith("Package."):
                    HandlePackage(migCommand, request);
                    break;

                case string cmd when cmd.StartsWith("Widgets."):
                    HandleWidgets(migCommand, request);
                    break;

                case string cmd when cmd.StartsWith("Groups."):
                    HandleGroups(migCommand, request);
                    break;

                case string cmd when cmd.StartsWith("Stores."):
                    HandleStores(migCommand, request);
                    break;

                case string cmd when cmd.StartsWith("Modules."):
                    HandleModules(migCommand, request);
                    break;

                case string cmd when cmd.StartsWith("Interface"):
                    HandleInterfaces(migCommand, request);
                    break;
        ...
        private void HandlePackage(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            switch (migCommand.Command)
            {
                case "Package.Get":
                {
                    var pkgSourceUrl = migCommand.GetOption(0);
                    var pkg = _homegenie.PackageManager.GetInstalledPackage(pkgSourceUrl);
                ...
                {
                    var pkgSourceUrl = migCommand.GetOption(0);
                    var success = _homegenie.PackageManager.InstallPackage(pkgSourceUrl);
                    if (success)
                    {
        ...
        private void HandleWidgets(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            switch (migCommand.Command)
            {
                case "Widgets.List":
                ...
                case "Widgets.Import":
                {
                    ...
                    request.ResponseData = _homegenie.PackageManager.WidgetImport(archiveFile, importPath)
                        ? new ResponseText("OK")
                        : new ResponseText("ERROR");
                }
                    break;
        ...
        private void HandleGroups(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            switch (migCommand.Command)
            {
                ...
                case "Groups.SortModules":
                {
                    ...
                    var sortGroup = _homegenie.GetGroups(migCommand.GetOption(0)).Find(x => x.Name == groupName);
                    if (sortGroup != null)
                    {
                ...
                case "Groups.WallpaperList":
                    ...
                    foreach (var image in images)
                        wallpaperList.Add(Path.GetFileName(image));
        ...
        private void HandleStores(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            string response;
            switch (migCommand.Command)
            {
        ...
        private void HandleModules(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            switch (migCommand.Command)
            {
                case "Modules.Get":
                    ...
                    for (var i = 0; i < newModules.Count; i++)
                    {
                        try
                        {
                            ...
                            for (var p = 0; p < moduleProperties.Count; p++)
                            {
                                ...
                                parameter = module.Properties.Find(delegate(ModuleParameter mp) { return mp.Name == propertyName; });
                    ...
                    else
                    {
                        ...
                        foreach (var newParameter in newModule.Properties)
                        {
                            ...
                            if (currentParameter == null)
                            {
                                ...
                                _homegenie.RaiseEvent(Domains.HomeGenie_System, currentModule.Domain, currentModule.Address, currentModule.Description, newParameter.Name,
                                    newParameter.Value);
                            }
                            else if (newParameter.NeedsUpdate)
                            {
                                ...
                                {
                                    _homegenie.RaiseEvent(Domains.HomeGenie_System, currentModule.Domain, currentModule.Address, currentModule.Description, Properties.MeterWatts,
                                        "0.0");
                                }
        ...
        private void HandleInterfaces(MigInterfaceCommand migCommand, MigClientRequest request)
        {
            string response;
            switch (migCommand.Command)
            {
                ...
                case "Interface.Install":
                    ...
                    request.ResponseData = _homegenie.PackageManager.InterfaceInstall(Path.Combine(_tempFolderPath, "mig"))
                        ? new ResponseText("OK")
                        : new ResponseText("NOT A VALID ADD-ON PACKAGE");
                    break;
            }
        }
